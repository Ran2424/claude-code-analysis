...
本文档覆盖整个 `src/utils/` 目录（36 个子目录、564 个文件），以及 `src/interactiveHelpers.tsx` 和 `src/ink.ts`。

## 目录

1. [核心数据结构](#1-core-data-structures)
2. [Shell 与进程执行](#2-shell--process-execution)
3. [Bash 解析与分析](#3-bash-parsing--analysis)
4. [配置与设置](#4-configuration--settings)
5. [认证与授权管理](#5-authentication--auth-management)
6. [权限系统](#6-permissions-system)
7. [文件与文件系统工具](#7-file--filesystem-utilities)
8. [Git 工具](#8-git-utilities)
9. [会话管理](#9-session-management)
10. [消息与内容工具](#10-message--content-utilities)
11. [模型工具](#11-model-utilities)
12. [MCP 工具](#12-mcp-utilities)
13. [Swarm / 多智能体系统](#13-swarm--multi-agent-system)
14. [任务与输出管理](#14-task--output-management)
15. [处理用户输入](#15-process-user-input)
16. [Hooks 系统](#16-hooks-system)
17. [分析与遥测](#17-analytics--telemetry)
18. [后台与 Cron 系统](#18-background--cron-systems)
19. [Computer Use](#19-computer-use)
20. [Claude-in-Chrome](#20-claude-in-chrome)
21. [深度链接系统](#21-deep-link-system)
22. [插件工具](#22-plugin-utilities)
23. [Swarm 后端](#23-swarm-backends)
24. [Skills 与建议](#24-skills--suggestions)
25. [Todo 系统](#25-todo-system)
26. [Teleport 系统](#26-teleport-system)
27. [内存工具](#27-memory-utilities)
28. [沙盒工具](#28-sandbox-utilities)
29. [安全与安全存储](#29-security--secure-storage)
30. [原生安装器](#30-native-installer)
31. [通用工具（顶层）](#31-general-utilities-top-level)
...
### `QueryGuard.ts`
用于 query 生命周期（`idle` / `dispatching` / `running`）的同步状态机，兼容 React 的 `useSyncExternalStore`。
...
```
在整个代码库中约使用 15 次，用于状态变更事件的发布/订阅。
...
- 文件模式（bash）：stdout+stderr 都写进同一个文件 fd（原子 `O_APPEND`）
- 管道模式（hooks、`onStdout`）：数据通过 `StreamWrapper → TaskOutput` 保存在内存里
- CWD 跟踪：每条命令后把 `pwd -P` 输出写到临时文件
- 沙盒包装：当 `shouldUseSandbox` 为真时调用 `SandboxManager.wrapWithSandbox()`
- 沙盒中的 PowerShell 会用 `/bin/sh` 作为外层 shell，并把内部命令编码成 base64
- 命令结束后：读取 cwd 文件、更新状态、通知 hooks、清理临时资源
- Windows 路径通过 `posixPathToWindowsPath()` 处理

### `ShellCommand.ts`
把子进程封装成 `ShellCommand` 接口。
...
### `abortController.ts`
使用 WeakRef 的内存安全 AbortController 工具。
...
### `bash/specs/`
按命令拆分的补全规格：`alias.ts`、`nohup.ts`、`pyright.ts`、`sleep.ts`、`srun.ts`、`time.ts`、`timeout.ts`。

### `bash/treeSitterAnalysis.ts`
基于 Tree-sitter 的 bash AST 深度分析，用于命令分类。
...
### `config.ts`
核心配置文件。负责管理全局配置（按用户）、项目配置（按项目）和会话配置。
...
Claude Code 全局使用的 Ink 终端渲染框架公共 API。

**用途：** 用 `ThemeProvider` 包裹内部 `ink/root.ts`，让每次渲染都自动带上主题。
...
## 34. `utils/shell/` — Shell Provider 抽象
...
## 35. `utils/ultraplan/` — Ultraplan 远程会话轮询
